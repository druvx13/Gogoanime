...
if ($apiClient) {
    ...
    error_log("ApiClient not available in status/ongoing.php.");
...
$template_vars = [
    ...
    'status_anime_items' => $status_anime_items,
...
try {
    ...
    if ($twig) {
        echo $twig->render('status-listing.html.twig', $template_vars);
    } else {
        error_log("Failed to get Twig instance from ConfigLoader in status/ongoing.php.");
        echo "Error: Could not initialize the templating engine.";
    }
} catch (\Throwable $e) {
    error_log("Error in status/ongoing.php: " . $e->getMessage() . " Trace: " . $e->getTraceAsString());
